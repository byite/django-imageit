...
from .models import TestScaleItModel, TestCropItModel, TestScaleItModelSmallSave
from .forms import TestScaleItForm, TestCropItForm

from imageit.settings import IMAGEIT_MAX_SAVE_SIZE_MB
#sys.stderr.write(repr('') + '\n')


class ScaleItModelFieldTestCase(TestCase):
    def setUp(self):
        with open('tests/static/img/test1000x1000.png', 'rb') as img:
            scaled_img = TestScaleItModel.objects.create(name='scaled_png', image=File(img))

        with open('tests/static/img/test1000x1000xxx.png', 'rb') as img:
            scaled_img = TestScaleItModel(name='scaled_png_saved', image=File(img))
            scaled_img.save()
        
        with open('tests/static/img/test_svg.svg', 'rb') as img:
    ...
    def test_invalid_file(self):
        with self.assertRaises(ValidationError):
            with open('tests/static/docs/test.pdf', 'rb') as img:
                scaled_img = TestScaleItModel.objects.create(name='test4', image=File(img))
                scaled_img.save()

    def test_script_injection(self):
        with self.assertRaises(ValidationError):
            with open('tests/static/img/test_svg_js.svg', 'rb') as img:
                scaled_img = TestScaleItModel.objects.create(name='test5', image=File(img))
                scaled_img.save()

    def test_upload_size_validation(self):
        with self.settings(IMAGEIT_MAX_SAVE_SIZE_MB=1):
            with self.assertRaises(ValidationError):
                with open('tests/static/img/test1000x1000.png', 'rb') as img:
                    scaled_img = TestScaleItModelSmallSave.objects.create(name='test5', image=File(img))
                    scaled_img.save()
